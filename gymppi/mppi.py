"""Gaussian-prior MPPI planner.

This module implements information-theoretic MPPI where the action prior and
controlled distribution are both Gaussian. It supports both real-environment
vectorized rollouts and learned transition-model rollouts, including ensemble
models using batch-wise member selection.
...
class MPPI():
    """Model Predictive Path Integral (MPPI) controller with Gaussian prior.

    Args:
        env: Base environment that defines observation/action spaces.
        rollout_envs: Optional vectorized envs used for trajectory rollouts.
        gamma: Discount factor used in trajectory costs.
        transition_model: Optional learned dynamics model for model rollouts.
        Q: Optional terminal value function.
        mu: Optional policy baseline used in specific control modes.
        B: Batch size (number of parallel planning problems).
        T: Planning horizon.
        K: Number of sampled trajectories per batch element.
        mean, cov: Mean/covariance of Gaussian noise.
        lambda_: Temperature parameter.
        u_init: Tail action used during warmstart rolling.
        control_mode: One of supported MPPI control update modes.
        handle_terminations: Whether to down-weight terminated rollouts.
    """
